...
export const remove = async (
  ...
  try {
    ...
    if (count > 0) return res.status(200).json({ message: "Tidak dapat dihapus! Pastikan prompt ini tidak digunakan dalam koneksi Whatsapp!" });
    ...
    return res.status(500).json({ message: "Tidak dapat dihapus! Pastikan prompt ini tidak digunakan!" });
  }
};
